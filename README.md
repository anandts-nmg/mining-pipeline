...
| 04 | Preliminary Prospect Delineation & Ranking | build | **implemented** — 250 m evidence-scoring grid on the v9 §5 8-criterion weighted matrix (contact-based geology, localized CMCS; **attribute-aware**: focused alteration activates `rs`, geochem-anomaly polygons drive `geochem`+`elements`); high-score cells dissolved into `BUD-PSP` A/B/C/D-ranked prospects + ranking table + Go/No-Go matrix; field/pXRF/drone score 0 at desktop (ceiling B); all *Preliminary — not ore proof*. See `PHASE_04_PLAN.md` |
